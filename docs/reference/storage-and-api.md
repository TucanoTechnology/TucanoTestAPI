...
| Runs | `GET /test_runs`, parent-scoped creation, listing and deletion (`GET`/`POST /projects/{id}/test_runs`, `DELETE /projects/{id}/test_runs/{run_id}`), `GET`/`PUT`/`DELETE /test_runs/{id}`, `POST /test_runs/{id}/duplicate`, suite and case inclusion (`/test_runs/{id}/test_suites`, `/test_runs/{id}/test_cases`), result recording and per-case update/removal (`POST /test_runs/{id}/results`, `PUT`/`DELETE /test_runs/{id}/results/{case_id}`), defect links (`/test_runs/{id}/results/{case_id}/defects`), imports (`POST /test_runs/{id}/import/junit`, `POST /test_runs/{id}/import/json`), configuration links (`/test_runs/{id}/configurations`) |
