...
`./data` folder at `/data` (Docker creates the folder on first run, but it must be *writable* by uid `10001` — startup otherwise refuses with an error naming the path and the uid, #355). Keep test data and secrets out
...
| [docs/security/audit-summary-s1-s4.md](docs/security/audit-summary-s1-s4.md) | The consolidated security-audit summary for epic #166 — every S1–S4 finding with its severity, the remediation ticket that owns it, and the current status of each |
