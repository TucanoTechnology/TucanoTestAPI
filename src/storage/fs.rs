...
use super::layout::{
    attachment_path, document_path, resource_dir, set_private_permissions, test_case_dir,
    unique_suffix,
};
use super::{Repository, Resource};

/// Filesystem-backed [`Repository`]; one JSON document per resource.
#[derive(Clone)]
...
impl FileRepository {
    pub fn new(root: impl Into<PathBuf>) -> io::Result<Self> {
        ...
        for resource in Resource::ALL {
            fs::create_dir_all(resource_dir(&root, resource))?;
        }
        Ok(Self { root })
    }
...
impl Repository for FileRepository {
    fn list(&self, resource: Resource) -> io::Result<Vec<String>> {
        let mut entries = fs::read_dir(resource_dir(&self.root, resource))?
            .filter_map(Result::ok)
            .filter_map(|entry| {
                let kind = entry.file_type().ok()?;
                if resource.is_directory_backed() {
                    kind.is_dir()
    ...
    fn read(&self, resource: Resource, id: &str) -> io::Result<Value> {
        let path = document_path(&self.root, resource, id)?;
    ...
    fn exists(&self, resource: Resource, id: &str) -> io::Result<bool> {
        Ok(document_path(&self.root, resource, id)?.is_file())
    }

    fn write(&self, resource: Resource, id: &str, value: &Value) -> io::Result<()> {
        let lock = self.acquire_lock()?;
        let directory = resource_dir(&self.root, resource);
        fs::create_dir_all(&directory)?;
        let destination = document_path(&self.root, resource, id)?;
    ...
    fn delete(&self, resource: Resource, id: &str) -> io::Result<()> {
        let lock = self.acquire_lock()?;
        let result = if resource.is_directory_backed() {
            fs::remove_dir_all(test_case_dir(&self.root, id)?)
        } else {
            fs::remove_file(document_path(&self.root, resource, id)?)
        };
        lock.unlock()?;
        result
    }
    ...
    fn save_attachment(&self, id: &str, filename: &str, contents: &[u8]) -> io::Result<()> {
        let lock = self.acquire_lock()?;
        let directory = test_case_dir(&self.root, id)?;
        fs::create_dir_all(&directory)?;
        let path = attachment_path(&self.root, id, filename)?;
    ...
    fn read_attachment(&self, id: &str, filename: &str) -> io::Result<Vec<u8>> {
        fs::read(attachment_path(&self.root, id, filename)?)
    }

    fn delete_attachment(&self, id: &str, filename: &str) -> io::Result<()> {
        let lock = self.acquire_lock()?;
        let result = fs::remove_file(attachment_path(&self.root, id, filename)?);
        lock.unlock()?;
        result
    }
}
...
mod tests {
    ...
    fn new_creates_every_resource_directory() {
        ...
        for resource in Resource::ALL {
            assert!(
                directory.path().join(resource.dir_name()).is_dir(),
                "{resource:?}"
            );
        }
    }
    ...
    fn persists_json_atomically_and_rejects_traversal() {
        ...
        repository
            .write(Resource::Projects, "checkout.json", &value)
            .expect("write");
        assert_eq!(
            repository
                .read(Resource::Projects, "checkout.json")
                .expect("read"),
            value
        );
        assert!(
            repository
                .write(Resource::Projects, "../escape.json", &value)
                .is_err()
        );
    }
    ...
    fn list_returns_sorted_json_files_only() {
        let (directory, repository) = repository();
        repository
            .write(Resource::Projects, "beta.json", &json!({"name": "Beta"}))
            .expect("write beta");
        repository
            .write(Resource::Projects, "alpha.json", &json!({"name": "Alpha"}))
            .expect("write alpha");
    ...
    fn list_is_empty_for_new_storage() {
        let (_directory, repository) = repository();
        assert!(repository.list(Resource::Runs).expect("list").is_empty());
    }
    ...
    fn test_cases_are_stored_as_directories() {
        let (directory, repository) = repository();
        repository
            .write(Resource::Cases, "TC-001", &json!({"testCaseId": "TC-001"}))
            .expect("write");
        ...
        assert_eq!(
            repository.list(Resource::Cases).expect("list"),
            vec!["TC-001".to_owned()]
        );
    }
    ...
    fn exists_reflects_stored_documents() {
        ...
        assert!(
            !repository
                .exists(Resource::Projects, "missing.json")
                .expect("miss")
        );
        repository
            .write(Resource::Projects, "found.json", &json!({"name": "Found"}))
            .expect("write");
        assert!(
            repository
                .exists(Resource::Projects, "found.json")
                .expect("hit")
        );
    }
    ...
    fn read_reports_missing_documents() {
        ...
        let error = repository
            .read(Resource::Projects, "missing.json")
            .expect_err("miss");
    ...
    fn read_reports_corrupted_json_as_invalid_data() {
        ...
        let error = repository
            .read(Resource::Projects, "broken.json")
            .expect_err("corrupt");
    ...
    fn delete_reports_missing_documents() {
        let (_directory, repository) = repository();
        let error = repository
            .delete(Resource::Projects, "missing.json")
            .expect_err("miss");
        assert_eq!(error.kind(), io::ErrorKind::NotFound);
    }
    ...
    fn flat_resources_require_a_json_extension() {
        let (_directory, repository) = repository();
        let error = repository
            .write(Resource::Projects, "checkout", &json!({"name": "Checkout"}))
            .expect_err("rejected");
    ...
    fn hostile_identifiers_are_rejected() {
        ...
        for id in [
            ...
            assert!(
                repository.read(Resource::Projects, id).is_err(),
    ...
    fn concurrent_writers_never_publish_partial_documents() {
        ...
        let writers = (0..8)
            .map(|index| {
                ...
                std::thread::spawn(move || {
                    writer
                        .write(Resource::Projects, "shared.json", &json!({"name": index}))
        ...
        let stored = readable
            .read(Resource::Projects, "shared.json")
            .expect("read");
